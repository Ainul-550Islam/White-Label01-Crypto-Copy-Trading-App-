...
# The order is deliberate: regenerate the ancestor chain in part order (each generator measures the
# tree as it stands and embeds the current text of every file it lists, so an ancestor written
# before Part 21's files existed would be stale), refresh the generated source views, and only then
# prove all six handovers byte-reproducible - a `--check` run before the dump would certify a tree
# state that the dump then changed.
...
npx prettier --check 'apps/api/src/**/*.ts' 'scripts/dr-*.mjs' 'docs/dr/manifest.json' 2>&1 | tail -2
...
python3 -m wlct_trading.observability.chaos --environment local >/dev/null 2>&1; echo "chaos local exit=$?"
python3 -m wlct_trading.observability.chaos --environment production >/dev/null 2>&1; echo "chaos production exit=$?"
